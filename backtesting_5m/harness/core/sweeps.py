...
def run_with_sweeps(investigation_dir, quote, execn, sample, output, episodes,
    ...
    data it read, not just the base run's. `model_dir` is forwarded to every
    arm too -- an investigation whose local block copies were dropped in
    favour of a shared model must have every arm resolve the same blocks the
    headline run does, or a sweep arm quietly falls through to
    `harness/blocks/defaults/` instead.
    """
    base = run(investigation_dir, quote, execn, sample, output, episodes,
               inputs=inputs, model_dir=model_dir)
    ...
    for name, arm_spec in FILL_ARMS.items():
        ...
        arm = run(investigation_dir, quote, arm_execn, sample,
                  output, episodes, inputs=inputs, model_dir=model_dir)
