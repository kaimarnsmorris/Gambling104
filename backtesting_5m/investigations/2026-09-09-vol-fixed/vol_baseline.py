...
IN-WINDOW observations only. Two reasons. It is the one thing that must stay
identical across `vol_baseline.py`, `vol.py` and `vol_flat.py` or the three
arms stop quoting on the same indices and the comparison stops being about
sigma; and it costs almost nothing to leave -- measured over the 568 markets
of 2026-08-19..20, the gate clears at in-window index 6 (0.6 s) at the median
and index 8 at the 99th percentile, 0.34 % of the window on average. Letting
the warm-up satisfy it would buy back a third of a second and change what all
three arms trade.
