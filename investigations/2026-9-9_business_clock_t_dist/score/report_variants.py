...
FINDINGS = {
    "alpha_half": (
        ...
        "reason: top-of-book imbalance is I = ln(ask_size / bid_size) - the sign "
        "scripts/12_alpha.py actually fitted (np.log(av / bv), ask volume over bid "
        "volume), which is why the saturated beta0 is NEGATIVE at -0.19 bp per unit "
        "of I - and it needs SIZES, which the Polymarket 5 m book panel this export "
        "reads "
    ...
    "alpha_x2": (
        ...
        "ln(ask_size/bid_size), the fitted sign) and this panel has none - see the "
        "alpha_half entry "
    ...
    "basis_alt": (
        "FIX-ROUND 2, corrected: an earlier version of this finding, and of the "
        "variant's own note, claimed basis_alt moves 'eps_bar and the carry' and "
        "also var_eps. Measured on this export, only the LEVEL channel moves: "
        "eps_bar (max |delta| 6.6e-05), and p_ref / s / sigma because "
        "p_ref = exp(x_t + b_t) carries the tracked level as a scale factor. "
        "carry, var_eps, var_basis and m_Y are ALL bit-for-bit identical to "
        "baseline, and Var_Y itself agrees to 1.1e-15 relative. That is the right "
        "answer rather than a shortfall: the Chainlink carry is built from win.F "
        "and x_t, which carry no basis at all, and var_eps = q_unit * "
        "sigma_at(v_loc)^2 never sees which tracker supplied the level. The note "
        "in variants/basis_alt.yaml has been corrected to match. The variant "
        "itself performs well - second-best near30 of the 16 (-0.0312) and one of "
        "five that improve pooled log-loss (-0.0015)."),
    "eps_x0": (
        ...
        "FIX-ROUND 2, RE-SCORED: eps_scale = 0 used to switch off var_basis as "
        "well, because both pricing paths gated the whole residual block on "
        "model.eps.sigma_bp != 0.0 - but the basis-drift variance is a different "
        "process with its own selector (basis_tracker). Measured at n = 60, "
        "baseline var_basis = 4.011e-11 and this variant's was exactly 0.0, about "
        "13% of its whole Var_Y reduction at that horizon and 41% at n = 120. "
        "With the basis channel restored, eps_x0's damage to pooled log-loss "
        "nearly halves: +0.00243 -> +0.00158 against baseline (0.408421 -> "
        "0.407571). near30 is essentially unmoved (0.526375 -> 0.526491, "
        "+0.0206), and so is the 1-5 s level ratio (0.3116 -> 0.3116) - the basis "
        "drift over a five-second window is negligible, which is why the "
        "correction shows up at long tte and not in the cell the note is about."),
    "eps_x2": (
    ...
    "shrink_05": (
        "FIX-ROUND 2, RE-DERIVED after a real bug in the shrink target. "
        "curve.unconditional_xi differenced its integral against a prepended "
        "zero, so xi_bar[0] was the whole HEAD INTEGRAL rather than the block's "
        "first increment g(H+1) - g(H) - measured, 161x too large at n = 300. It "
        "bit only where H > 0 (n > 180, i.e. t_s < 120, the first 121 of each "
        "market's 301 rows), and shrink_05 is the only shipped variant with "
        "shrink_w != 0, so it is the only variant that was affected. Rebuilt on "
        "the fixed target, the contaminated rows move materially - Var_Y by a "
        "median 1.21% and up to 5.15% over t_s < 120 - while the t_s >= 120 rows "
        "move only by float noise (max 3.1e-15 relative), exactly as the "
        "diagnosis predicts. "
        "THE CONCLUSION DOES NOT FLIP: pooled log-loss goes 0.404284 -> 0.404329, "
        "so the improvement against baseline softens from -0.00170 to -0.00166 "
        "and shrink_05 still improves pooled log-loss. near30 log-loss is "
        "unchanged to six decimals (0.523325, +0.0174 against baseline) because "
        "that cell is tte <= 30 s, which is entirely H == 0 territory - the "
        "contaminated head never reached it. The note's own framing survives: "
        "shrink_05 remains the milder remedy for the same short-horizon complaint "
        "as short_vol_minus20, with smaller near30 damage (+0.0174 vs +0.0358). "
        "One claim in the previous finding was wrong independently of the bug and "
        "is withdrawn: shrink_05 is NOT 'one of only two' variants improving "
        "pooled log-loss. Five of the sixteen do - basis_alt (-0.0015), shrink_05 "
        "(-0.0017), vol_plus10 (-0.0009), thin_tails (-0.0008) and eps_x2 "
        "(-0.0006) - and three of those improve near30 as well, which shrink_05 "
        "does not."),
    "vol_minus10": (
